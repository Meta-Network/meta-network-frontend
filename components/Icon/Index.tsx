import Icon from '@ant-design/icons'

const EmailSvg = () => (
  <svg className="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="1715"><path d="M513 583.8l448.5-448.5c-11.6-4.7-24.3-7.3-37.5-7.3H100c-12.7 0-24.9 2.4-36.1 6.7L513 583.8z" fill="" p-id="1716"></path><path d="M513 674.3L14.6 175.9C5.3 191.1 0 208.9 0 228v568c0 55.2 44.8 100 100 100h824c55.2 0 100-44.8 100-100V228c0-18.5-5.1-35.9-13.9-50.8L513 674.3z" fill="" p-id="1717"></path></svg>
)
export const EmailIcon: React.FC = props => <Icon component={EmailSvg} {...props} />

const USvg = () => (
  <svg />
)
export const UIcon: React.FC = props => <Icon component={USvg} {...props} />

const SearchSvg = () => (
  <svg className="icon" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg" fill="currentColor"><path d="M192 480a256 256 0 1 1 512 0 256 256 0 0 1-512 0m631.776 362.496-143.2-143.168A318.464 318.464 0 0 0 768 480c0-176.736-143.264-320-320-320S128 303.264 128 480s143.264 320 320 320a318.016 318.016 0 0 0 184.16-58.592l146.336 146.368c12.512 12.48 32.768 12.48 45.28 0 12.48-12.512 12.48-32.768 0-45.28" /></svg>
)
export const SearchIcon: React.FC = props => <Icon component={SearchSvg} {...props} />

const SwitchVerticalSvg = () => (
  <svg className="icon" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg" fill="currentColor"><path d="M469.333 384c-12.8 0-21.333-4.267-29.866-12.8l-140.8-140.8-140.8 140.8c-17.067 17.067-42.667 17.067-59.734 0s-17.066-42.667 0-59.733L268.8 140.8c17.067-17.067 42.667-17.067 59.733 0L499.2 311.467c17.067 17.066 17.067 42.666 0 59.733-8.533 8.533-17.067 12.8-29.867 12.8z" /><path d="M298.667 597.333c-25.6 0-42.667-17.066-42.667-42.666v-384c0-25.6 17.067-42.667 42.667-42.667s42.666 17.067 42.666 42.667v384c0 25.6-17.066 42.666-42.666 42.666zM725.333 896c-12.8 0-21.333-4.267-29.866-12.8L524.8 712.533c-17.067-17.066-17.067-42.666 0-59.733s42.667-17.067 59.733 0l140.8 140.8 140.8-140.8c17.067-17.067 42.667-17.067 59.734 0s17.066 42.667 0 59.733L755.2 883.2c-8.533 8.533-17.067 12.8-29.867 12.8z" /><path d="M725.333 896c-25.6 0-42.666-17.067-42.666-42.667V426.667c0-25.6 17.066-42.667 42.666-42.667S768 401.067 768 426.667v426.666c0 25.6-17.067 42.667-42.667 42.667z" /></svg>
)
export const SwitchVerticalIcon: React.FC = props => <Icon component={SwitchVerticalSvg} {...props} />

const BookmarkSvg = () => (
  <svg className="icon" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg" fill="currentColor"><path d="M804.907 981.333c-12.886 0-25.814-4.693-33.878-14.08L512.811 715.392 254.592 967.253c-14.507 14.08-35.499 18.774-53.248 10.966-19.37-7.851-30.677-25.046-30.677-43.819V180.352c0-76.672 62.933-137.685 142.037-137.685h398.592c77.483 0 142.037 61.013 142.037 137.685V934.4c0 18.773-11.306 35.968-30.677 43.819-4.821 3.114-11.264 3.114-17.75 3.114zM511.189 604.288c12.928 0 25.814 4.693 33.878 14.08L754.9 821.76V180.352c1.579-25.045-19.37-43.819-43.648-43.819H312.747c-25.856 0-45.227 18.774-45.227 43.819V821.76l209.835-203.392c9.685-9.387 20.992-14.08 33.877-14.08z" /></svg>
)
export const BookmarkIcon: React.FC = props => <Icon component={BookmarkSvg} {...props} />

const BookmarkFillSvg = () => (
  <svg className="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="8814"><path d="M853.333333 896a42.666667 42.666667 0 0 1-65.28 36.266667l-253.44-158.72a42.666667 42.666667 0 0 0-45.226666 0l-253.44 158.72A42.666667 42.666667 0 0 1 170.666667 896V170.666667a85.333333 85.333333 0 0 1 85.333333-85.333334h512a85.333333 85.333333 0 0 1 85.333333 85.333334z" p-id="8815"></path></svg>
)
export const BookmarkFillIcon: React.FC = props => <Icon component={BookmarkFillSvg} {...props} />

const ArrowTopLeftSvg = () => (
  <svg className="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="9756"><path d="M512 714.666667c-8.533333 0-17.066667-2.133333-23.466667-8.533334l-341.333333-341.333333c-12.8-12.8-12.8-32 0-44.8 12.8-12.8 32-12.8 44.8 0l320 317.866667 317.866667-320c12.8-12.8 32-12.8 44.8 0 12.8 12.8 12.8 32 0 44.8L533.333333 704c-4.266667 8.533333-12.8 10.666667-21.333333 10.666667z" p-id="9757"></path></svg>
)
export const ArrowTopLeftIcon: React.FC = props => <Icon component={ArrowTopLeftSvg} {...props} />

const InviteSvg = () => (
  <svg />
)
export const InviteIcon: React.FC = props => <Icon component={InviteSvg} {...props} />

const LogoutSvg = () => (
  <svg />
)
export const LogoutIcon: React.FC = props => <Icon component={LogoutSvg} {...props} />

const SortTopSvg = () => (
  <svg className="icon" viewBox="0 0 18 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M1 1h11.375M1 4.5h7.875M1 8h5.25m3.5 0 3.5-3.5m0 0 3.5 3.5m-3.5-3.5V15" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" /></svg>
)
export const SortTopIcon: React.FC = props => <Icon component={SortTopSvg} {...props} />

const SortDoneSvg = () => (
  <svg className="icon" viewBox="0 0 16 14" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M15.108 12.75h-13m13-4h-13m13-4h-6m-2-3.5-4 4-2.5-2.5" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" /></svg>
)
export const SortDoneIcon: React.FC<any> = props => <Icon component={SortDoneSvg} {...props} />

const CopySvg = () => (
  <svg />
)
export const CopyIcon: React.FC = props => <Icon component={CopySvg} {...props} />

const CircleSvg = () => (
  <svg className="icon" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M7.86 2h8.28L22 7.86v8.28L16.14 22H7.86L2 16.14V7.86L7.86 2z" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" strokeDasharray="4 4" /></svg>
)
export const CircleIcon: React.FC = props => <Icon component={CircleSvg} {...props} />

const CircleSuccessSvg = () => (
  <svg className="icon" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M7.86 2h8.28L22 7.86v8.28L16.14 22H7.86L2 16.14V7.86L7.86 2z" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" /><path d="m17.5 8-7.219 7.875L7 12.295" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" /></svg>
)
export const CircleSuccessIcon: React.FC = props => <Icon component={CircleSuccessSvg} {...props} />

const CircleWarningSvg = () => (
  <svg className="icon" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M7.86 2h8.28L22 7.86v8.28L16.14 22H7.86L2 16.14V7.86L7.86 2z" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" strokeDasharray="4 4" /><path d="M12 16h.01M12 8v4" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg>
)
export const CircleWarningIcon: React.FC = props => <Icon component={CircleWarningSvg} {...props} />

const CircleEmptySvg = () => (
  <svg className="icon" viewBox="0 0 25 25" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M7.86 2.5h8.28L22 8.36v8.28l-5.86 5.86H7.86L2 16.64V8.36L7.86 2.5Z" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" strokeDasharray="4 4" /></svg>
)
export const CircleEmptyIcon: React.FC = props => <Icon component={CircleEmptySvg} {...props} />

const CloseModalSvg = () => (
  <svg className="icon" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" clipRule="evenodd" d="M15.108 29.4c7.953 0 14.4-6.447 14.4-14.4S23.061.6 15.108.6.708 7.047.708 15s6.447 14.4 14.4 14.4Zm-2.327-19.273a1.8 1.8 0 1 0-2.546 2.546L12.563 15l-2.328 2.327a1.8 1.8 0 1 0 2.546 2.546l2.327-2.327 2.328 2.327a1.8 1.8 0 0 0 2.545-2.546L17.654 15l2.327-2.327a1.8 1.8 0 1 0-2.546-2.546l-2.327 2.327-2.327-2.327Z" fill="currentColor" /></svg>
)
export const CloseModalIcon: React.FC<any> = props => <Icon component={CloseModalSvg} {...props} />

const ToastWarningSvg = () => (
  <svg className="icon" viewBox="0 0 23 23" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M7.36 1.5H15.64L21.5 7.36V15.64L15.64 21.5H7.36L1.5 15.64V7.36L7.36 1.5Z" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" />
    <path d="M11.5 15.5H11.51" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
    <path d="M11.5 7.5V11.5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
  </svg>

)
export const ToastWarningIcon: React.FC = props => <Icon component={ToastWarningSvg} {...props} />

const ToastSuccessSvg = () => (
  <svg className="icon" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M6.86 1H15.14L21 6.86V15.14L15.14 21H6.86L1 15.14V6.86L6.86 1Z" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" />
    <path d="M16.5 7L9.28125 14.875L6 11.2955" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" />
  </svg>
)
export const ToastSuccessIcon: React.FC = props => <Icon component={ToastSuccessSvg} {...props} />

const SliderHomeSvg = () => (
  <svg />
)
export const SliderHomeIcon: React.FC = props => <Icon component={SliderHomeSvg} {...props} />

const SliderSpaceSvg = () => (
  <svg className="icon" viewBox="0 0 20 18" xmlns="http://www.w3.org/2000/svg"><path fill="currentColor" d="M18.344 0H1.654C.772 0 .058.714.058 1.598v13.85c0 .883.714 1.597 1.598 1.597h16.69c.884 0 1.598-.714 1.598-1.598V1.598C19.943.714 19.23 0 18.345 0h-.001ZM5.75 15.63H1.48V6.393h4.27v9.239Zm12.753 0H7.162V6.393h11.364c-.006 6.16-.015 9.239-.023 9.239Zm.023-10.44H1.48V1.423h17.046V5.19Z" fillRule="nonzero" /></svg>
)
export const SliderSpaceIcon: React.FC<any> = props => <Icon component={SliderSpaceSvg} {...props} />

const SliderInviteSvg = () => (
  <svg className="icon" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
    <g fill="currentColor" fillRule="nonzero">
      <path d="M14.48.008a1.996 1.996 0 0 1 1.995 1.996v2.994h1.496a1.996 1.996 0 0 1 1.997 1.996V17.97a1.996 1.996 0 0 1-1.997 1.997H2.004A1.995 1.995 0 0 1 .008 17.97V6.994a1.996 1.996 0 0 1 1.996-1.996H3.5V2.004A1.996 1.996 0 0 1 5.497.008h8.982Zm3.991 9.227-7.402 5.842a1.746 1.746 0 0 1-2.164 0l-7.4-5.843v8.737a.501.501 0 0 0 .498.5h15.968a.499.499 0 0 0 .5-.5V9.235Zm-3.992-7.73H5.497a.5.5 0 0 0-.5.499v8.08l4.836 3.818a.25.25 0 0 0 .31 0l4.835-3.817V2.004a.5.5 0 0 0-.499-.5ZM3.5 6.495H2.004a.5.5 0 0 0-.5.499v.333l1.997 1.576V6.495H3.5Zm14.47 0h-1.496v2.407l1.996-1.575v-.333a.5.5 0 0 0-.5-.5Z" /><path d="M7.742 4.998a.748.748 0 1 1 0 1.497.748.748 0 0 1 0-1.497Zm2.496 0a.748.748 0 1 1 0 1.496.748.748 0 0 1 0-1.496Zm2.494 0a.749.749 0 1 1 0 1.497.749.749 0 0 1 0-1.497Z" />
    </g>
  </svg>
)
export const SliderInviteIcon: React.FC<any> = props => <Icon component={SliderInviteSvg} {...props} />

const SliderShareSvg = () => (
  <svg className="icon" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg"><path d="M755.84 915.2H271.36c-74.24 0-134.4-60.16-134.4-134.4V295.68c0-74.24 60.16-134.4 134.4-134.4h236.8c17.92 0 32 14.08 32 32s-14.08 32-32 32h-236.8c-39.04 0-70.4 31.36-70.4 70.4V780.8c0 39.04 31.36 70.4 70.4 70.4h485.12c39.04 0 70.4-31.36 70.4-70.4V535.04c0-17.92 14.08-32 32-32s32 14.08 32 32V780.8c0 74.24-60.8 134.4-135.04 134.4zm103.04-553.6c-17.92 0-32-14.08-32-32V225.28H720c-17.92 0-32-14.08-32-32s14.08-32 32-32h138.88c17.92 0 32 14.08 32 32V329.6c0 17.92-14.72 32-32 32z" fill="currentColor" /><path d="M503.04 581.12c-8.32 0-16.64-3.2-22.4-9.6-12.8-12.8-12.8-32.64 0-45.44L814.72 192c12.8-12.8 32.64-12.8 45.44 0s12.8 32.64 0 45.44L525.44 572.16c-6.4 5.76-14.08 8.96-22.4 8.96z" fill="currentColor" /></svg>
)
export const SliderShareIcon: React.FC<any> = props => <Icon component={SliderShareSvg} {...props} />

const SliderHexagonSvg = () => (
  <svg className="icon" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg"><path fill="currentColor" d="M512 981.333a130.56 130.56 0 0 1-64-17.066L149.333 793.6a128 128 0 0 1-64-110.933V341.333a128 128 0 0 1 64-110.933L448 59.733a128 128 0 0 1 128 0L874.667 230.4a128 128 0 0 1 64 110.933v341.334a128 128 0 0 1-64 110.933L576 964.267a130.56 130.56 0 0 1-64 17.066zM512 128a42.667 42.667 0 0 0-21.333 5.973L192 304.64a42.667 42.667 0 0 0-21.333 36.693v341.334A42.667 42.667 0 0 0 192 719.36l298.667 170.667a42.667 42.667 0 0 0 42.666 0L832 719.36a42.667 42.667 0 0 0 21.333-36.693V341.333A42.667 42.667 0 0 0 832 304.64L533.333 133.973A42.667 42.667 0 0 0 512 128zm384 554.667z" /></svg>
)
export const SliderHexagonIcon: React.FC<any> = props => <Icon component={SliderHexagonSvg} {...props} />


const TelegramSvg = () => (
  <svg className="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="2369"><path d="M881.706667 158.592l-756.48 291.712c-51.626667 20.736-51.328 49.536-9.472 62.378667l194.218666 60.586666 449.365334-283.52c21.248-12.928 40.661333-5.973333 24.704 8.192l-364.074667 328.576h-0.085333l0.085333 0.042667-13.397333 200.192c19.626667 0 28.288-9.002667 39.296-19.626667l94.336-91.733333 196.224 144.938667c36.181333 19.925333 62.165333 9.685333 71.168-33.493334l128.810666-607.061333c13.184-52.864-20.181333-76.8-54.698666-61.184z" p-id="2370"></path></svg>
)
export const TelegramIcon: React.FC<any> = props => <Icon component={TelegramSvg} {...props} />


const DiscordSvg = () => (
  <svg className="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="2969"><path d="M606.4 428.8c-28.8 0-52.8 25.6-52.8 57.6s24 57.6 52.8 57.6c28.8 0 52.8-25.6 52.8-57.6s-24-57.6-52.8-57.6zM419.2 428.8c-28.8 0-52.8 25.6-52.8 57.6s24 57.6 52.8 57.6c28.8 0 52.8-25.6 52.8-57.6s-24-57.6-52.8-57.6z" p-id="2970"></path><path d="M854.4 0H169.6C110.4 0 64 46.4 64 105.6v692.8c0 57.6 46.4 105.6 105.6 105.6h580.8l-27.2-94.4 65.6 60.8 62.4 57.6 108.8 96V105.6C960 46.4 913.6 0 854.4 0zM657.6 668.8s-19.2-22.4-33.6-41.6c67.2-19.2 92.8-60.8 92.8-60.8-20.8 14.4-41.6 24-59.2 30.4-25.6 11.2-49.6 17.6-73.6 22.4-49.6 9.6-94.4 6.4-132.8 0-28.8-6.4-54.4-14.4-75.2-22.4-11.2-4.8-24-9.6-36.8-17.6-1.6-1.6-3.2-1.6-4.8-3.2-1.6 0-1.6-1.6-1.6-1.6-9.6-4.8-14.4-8-14.4-8s24 41.6 89.6 60.8c-16 19.2-33.6 43.2-33.6 43.2-113.6-3.2-156.8-78.4-156.8-78.4 0-164.8 73.6-299.2 73.6-299.2 73.6-56 144-54.4 144-54.4l4.8 6.4C347.2 272 304 312 304 312s11.2-6.4 30.4-14.4c54.4-24 97.6-30.4 116.8-32 3.2 0 6.4-1.6 8-1.6 32-4.8 67.2-4.8 104-1.6 48 6.4 100.8 19.2 153.6 49.6 0 0-40-38.4-128-65.6l6.4-8s70.4-1.6 144 54.4c0 0 73.6 134.4 73.6 299.2 1.6-1.6-41.6 73.6-155.2 76.8z" p-id="2971"></path></svg>
)
export const DiscordIcon: React.FC<any> = props => <Icon component={DiscordSvg} {...props} />


const MatrixSvg = () => (
  <svg className="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="2475">
    <path fill="currentColor" d="M691.8144 616.2432v108.544H161.5872l-18.432 15.9744-55.9104 48.3328V353.28h116.3264v-65.536H21.7088v644.9152l164.2496-142.336h571.392v-174.08h-65.536z" p-id="2476"></path>
    <path fill="currentColor" d="M138.24 91.3408v590.4384h671.1296l192.9216 167.1168V91.3408H138.24z m798.5152 614.1952l-84.5824-73.3184-18.432-15.9744H203.776V156.8768h732.9792v548.6592z" p-id="2477"></path>
    <path fill="currentColor" d="M365.7728 287.744h65.536v180.0192h-65.536zM691.8144 287.744h65.536v180.0192h-65.536z" p-id="2478"></path>
  </svg>
)
export const MatrixIcon: React.FC = props => <Icon component={MatrixSvg} {...props} />


const PlayBackSvg = () => (
  <svg className="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="3463"><path fill="currentColor" d="M457.2928 292.992v328.4736h273.792v-91.2384h-186.1376v-237.2352h-87.6544zM512 19.2A490.752 490.752 0 0 0 198.5024 131.7632l23.5264-90.24L141.4912 19.2l-45.056 172.7488-22.4 86.3232 80.6144 22.2464 171.264 47.1552 22.4768-86.3744-99.6096-27.4432C317.4912 168.8832 409.984 128.6656 512 128.6656c211.712 0 383.3344 171.648 383.3344 383.3344 0 211.712-171.648 383.2576-383.3344 383.2576-211.712 0-383.1808-171.5712-383.1808-383.2576 0-10.8544 2.2528-21.0432 3.1488-31.6928l-110.6688-9.216A523.1872 523.1872 0 0 0 19.2 512C19.2 784.1792 239.8208 1004.8 512 1004.8S1004.8 784.1792 1004.8 512 784.1792 19.2 512 19.2z" p-id="3464"></path></svg>
)
export const PlayBackIcon: React.FC = props => <Icon component={PlayBackSvg} {...props} />

const MetaLogoSvg = () => (
  <svg viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M49.394 88.1062C72.7502 88.1062 91.6842 69.1722 91.6842 45.816C91.6842 22.4598 72.7502 3.52588 49.394 3.52588C26.0378 3.52588 7.10388 22.4598 7.10388 45.816C7.10388 69.1722 26.0378 88.1062 49.394 88.1062Z" fill="white"/>
    <path fill="currentColor" d="M49.9999 0C22.3857 0 0 22.3858 0 50C0 77.6143 22.3858 100 49.9999 100C77.6143 100 100 77.6143 100 50C100 22.3857 77.6143 0 49.9999 0ZM73.7947 64.8052C66.5275 75.5416 52.6565 80.4029 39.9203 75.7673C31.8547 72.8317 25.891 66.6844 22.9377 59.2958C22.4279 58.0205 23.6577 56.7345 24.9544 57.1886C31.117 59.3465 39.5324 61.1463 48.8966 62.1096C57.7716 63.0225 65.964 63.0256 72.3065 62.2929C73.6706 62.1355 74.5645 63.6679 73.7947 64.8052ZM85.2557 54.0063C80.3157 56.2153 65.3244 56.6576 49.7185 55.052C33.9936 53.4348 19.3188 49.9592 15.0316 46.7692C14.003 46.0039 13.4768 45.2473 13.5512 44.525C13.6975 43.1005 17.7006 42.1534 21.742 41.4678C21.9812 40.5236 22.2701 39.5821 22.6105 38.6469C28.0812 23.6162 44.7008 15.8664 59.7313 21.337C70.9851 25.4331 78.1561 35.779 78.7538 47.0285C83.2632 48.6474 87.4369 50.502 87.2764 52.0605C87.1997 52.8055 86.4873 53.4559 85.2557 54.0063Z" />
  </svg>
)
export const MetaLogoIcon: React.FC = props => <Icon component={MetaLogoSvg} {...props} />

const ElementSvg = () => (
  <svg viewBox="0 0 128 128" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path fill="currentColor" d="M64 0C28.656 0 0 28.656 0 64C0 99.344 28.656 128 64 128C99.344 128 128 99.344 128 64C128 28.656 99.344 0 64 0ZM56.992 25.1467C74.5333 25.1467 88.7573 39.3333 88.7573 56.8427C88.7573 58.086 88.2634 59.2784 87.3842 60.1576C86.5051 61.0368 85.3127 61.5307 84.0693 61.5307C82.826 61.5307 81.6336 61.0368 80.7544 60.1576C79.8752 59.2784 79.3813 58.086 79.3813 56.8427C79.3743 50.9112 77.0116 45.2255 72.8129 41.0358C68.6143 36.8461 62.9234 34.4957 56.992 34.5013C56.359 34.5316 55.7264 34.433 55.1326 34.2116C54.5388 33.9902 53.9961 33.6505 53.5374 33.2132C53.0787 32.7759 52.7135 32.25 52.464 31.6675C52.2146 31.0849 52.0859 30.4577 52.0859 29.824C52.0859 29.1903 52.2146 28.5631 52.464 27.9805C52.7135 27.398 53.0787 26.8721 53.5374 26.4348C53.9961 25.9975 54.5388 25.6578 55.1326 25.4364C55.7264 25.215 56.359 25.1164 56.992 25.1467ZM29.8347 75.8347C28.5932 75.8347 27.4024 75.3422 26.5235 74.4654C25.6447 73.5885 25.1495 72.3988 25.1467 71.1573C25.1502 66.9896 25.9746 62.8634 27.5729 59.0143C29.1713 55.1652 31.5121 51.6687 34.4619 48.7244C37.4116 45.7801 40.9125 43.4457 44.7645 41.8545C48.6165 40.2633 52.7443 39.4465 56.912 39.4507C58.1045 39.5205 59.2253 40.0434 60.0449 40.9124C60.8645 41.7814 61.3211 42.9308 61.3211 44.1253C61.3211 45.3199 60.8645 46.4693 60.0449 47.3383C59.2253 48.2073 58.1045 48.7302 56.912 48.8C50.9791 48.7958 45.2875 51.1481 41.0889 55.3398C36.8902 59.5315 34.5283 65.2191 34.5227 71.152C34.5198 72.394 34.0249 73.5842 33.1461 74.4619C32.2674 75.3396 31.0766 75.8333 29.8347 75.8347ZM71.088 102.859C53.5413 102.859 39.3173 88.6613 39.3173 71.1573C39.3748 69.9527 39.8938 68.8164 40.7667 67.9842C41.6396 67.1519 42.7993 66.6877 44.0053 66.6877C45.2114 66.6877 46.3711 67.1519 47.244 67.9842C48.1168 68.8164 48.6359 69.9527 48.6933 71.1573C48.6968 74.0947 49.2789 77.0027 50.4062 79.7152C51.5335 82.4276 53.1841 84.8915 55.2636 86.9661C57.3432 89.0407 59.811 90.6853 62.5261 91.8062C65.2413 92.9271 68.1506 93.5022 71.088 93.4987C71.7256 93.4614 72.3641 93.5548 72.9643 93.7731C73.5645 93.9914 74.1138 94.3301 74.5784 94.7683C75.0431 95.2066 75.4132 95.7351 75.6662 96.3216C75.9193 96.908 76.0498 97.54 76.0498 98.1787C76.0498 98.8174 75.9193 99.4493 75.6662 100.036C75.4132 100.622 75.0431 101.151 74.5784 101.589C74.1138 102.027 73.5645 102.366 72.9643 102.584C72.3641 102.803 71.7256 102.896 71.088 102.859ZM71.088 88.5493C69.8469 88.5493 68.6564 88.0578 67.7768 87.1822C66.8973 86.3067 66.4003 85.1184 66.3947 83.8773C66.3975 82.6344 66.8932 81.4434 67.7731 80.5655C68.6529 79.6877 69.8451 79.1947 71.088 79.1947C77.0209 79.2003 82.713 76.8493 86.9127 72.6586C91.1124 68.468 93.4756 62.7809 93.4827 56.848C93.4827 55.6047 93.9766 54.4123 94.8558 53.5331C95.7349 52.6539 96.9273 52.16 98.1707 52.16C99.414 52.16 100.606 52.6539 101.486 53.5331C102.365 54.4123 102.859 55.6047 102.859 56.848C102.859 74.352 88.6347 88.5493 71.088 88.5493Z" />
  </svg>

)
export const ElementIcon: React.FC = props => <Icon component={ElementSvg} {...props} />

const StorageSvg = () => (
  <svg className="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="12364">
    <path d="M141.824 493.568c0 67.5328 173.568 140.1856 370.176 140.1856v7.5776-7.5776c196.608 0 370.176-72.704 370.176-140.2368V419.584C796.16 471.04 656.0768 501.0944 512 501.0944S227.84 471.04 141.824 419.584v73.9328z m0 214.272c0 67.328 170.7008 147.6096 370.176 147.6096v7.5776-7.5776c199.4752 0 370.176-80.2816 370.176-147.6096v-77.4656c-86.0672 57.6-222.3104 92.416-370.176 92.416-147.8656 0-284.1088-34.816-370.176-92.416v77.4656zM512 161.024v7.5264c-206.0288 0-370.176 64.4096-370.176 121.7536S305.9712 412.0576 512 412.0576c206.0288 0 370.176-64.4096 370.176-121.7536S718.0288 168.5504 512 168.5504v-7.5776zM512 87.04v-7.5776c251.1872 0 460.8 83.7632 460.8 210.7904v417.536c0 129.7408-211.456 236.6464-460.8 236.6464s-460.8-106.9056-460.8-236.6464V290.304c0-126.976 209.6128-210.7904 460.8-210.7904v7.5776z" p-id="12365"></path>
  </svg>
)
export const StorageIcon: React.FC = props => <Icon component={StorageSvg} {...props} />

const GitBookSvg = () => (
  <svg className="icon" viewBox="0 0 64 48" version="1.1" xmlns="http://www.w3.org/2000/svg">
    <g id="Page-1" stroke="none" strokeWidth="1" fill="none" fillRule="evenodd">
      <g id="gitbook_logo_icon_168173" transform="translate(-1.000000, 0.000000)" fill="currentColor" fillRule="nonzero">
        <path d="M29.8,39.4 C30.8,39.4 31.7,40.2 31.7,41.3 C31.7,42.3 30.9,43.2 29.8,43.2 C28.8,43.2 27.9,42.4 27.9,41.3 C27.9,40.2 28.8,39.4 29.8,39.4 M59.2,27.8 C58.2,27.8 57.3,27 57.3,25.9 C57.3,24.9 58.1,24 59.2,24 C60.2,24 61.1,24.8 61.1,25.9 C61.1,26.9 60.2,27.8 59.2,27.8 M59.2,20.1 C56,20.1 53.4,22.7 53.4,25.9 C53.4,26.5 53.5,27.1 53.7,27.7 L34.6,37.9 C33.5,36.3 31.7,35.4 29.8,35.4 C27.6,35.4 25.6,36.7 24.6,38.6 L7.4,29.6 C5.6,28.6 4.2,25.7 4.4,22.9 C4.5,21.5 5,20.4 5.7,20 C6.2,19.7 6.7,19.8 7.4,20.1 L7.5,20.2 C12.1,22.6 27,30.4 27.6,30.7 C28.6,31.1 29.1,31.3 30.8,30.5 L61.6,14.5 C62.1,14.3 62.6,13.9 62.6,13.2 C62.6,12.3 61.7,11.9 61.7,11.9 C59.9,11.1 57.2,9.8 54.6,8.6 C49,6 42.6,3 39.8,1.5 C37.4,0.2 35.4,1.3 35.1,1.5 L34.4,1.8 C21.7,8.2 4.9,16.5 3.9,17.1 C2.2,18.1 1.1,20.2 1,22.8 C0.8,26.9 2.9,31.2 5.9,32.7 L24.1,42.1 C24.5,44.9 27,47.1 29.8,47.1 C33,47.1 35.5,44.6 35.6,41.4 L55.6,30.6 C56.6,31.4 57.9,31.8 59.2,31.8 C62.4,31.8 65,29.2 65,26 C65,22.7 62.4,20.1 59.2,20.1" id="Fill-1"></path>
      </g>
    </g>
  </svg>
)
export const GitBookIcon: React.FC = props => <Icon component={GitBookSvg} {...props} />
